...
async def create_deadline(deadline: DeadlineAdd, session: SessionDep):
    ...
    count_on_day = await DeadlineRepository.count_by_date(
        session, deadline.deadline_time
    )
    if count_on_day >= 3:
    ...
    if active_count >= 9:
        raise HTTPException(
            400, "Слишком много активных дедлайнов (максимум 9). Завершите часть."
        )
...
async def update_deadline(
    deadline_id: int, deadline_data: DeadlineUpdate, session: SessionDep
):
    ...
    if (
        ...
        count = await DeadlineRepository.count_by_date(
            session, deadline_data.deadline_time
        )
        if count >= 3:
    ...
    updated = await DeadlineRepository.update_deadline(
        deadline_id, deadline_data, session
    )
    if not updated:
        raise HTTPException(404, "Дедлайн не найден")
    return updated
...
async def update_deadline_status(
    deadline_id: int, is_completed: bool, session: SessionDep
):
