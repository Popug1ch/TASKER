...
def get_user_by_telegram_id(telegram_id: int):
    ...
    with SyncSession() as session:
        return session.query(UserModel).filter(UserModel.telegram_id == telegram_id).first()

def register_user_by_email_password(telegram_id: int, email: str, password: str) -> bool:
    """Привязать telegram_id к существующему пользователю по email и паролю"""
    with SyncSession() as session:
        user = session.query(UserModel).filter(UserModel.email == email).first()
        if not user or not UserRepository.verify_password(password, user.hashed_password):
            return False
...
def get_upcoming_deadlines(user_id: int):
    ...
    with SyncSession() as session:
        ...
        return session.query(DeadlineModel).filter(
        ...
        ).order_by(DeadlineModel.deadline_time).all()
...
def mark_task_complete(task_id: int, user_id: int) -> bool:
    ...
    with SyncSession() as session:
        task = session.query(TasksModel).filter(
            TasksModel.id == task_id,
            TasksModel.user_id == user_id
        ).first()
...
def get_upcoming_tasks_for_notification():
    ...
    with SyncSession() as session:
        ...
        tasks = session.query(TasksModel).filter(
            TasksModel.is_completed == False,
...
def get_deadlines_for_notification():
    ...
    with SyncSession() as session:
        now = datetime.now()
        deadlines = session.query(DeadlineModel).filter(
            DeadlineModel.is_completed == False,
            DeadlineModel.deadline_time > now
        ).all()
        result = []
