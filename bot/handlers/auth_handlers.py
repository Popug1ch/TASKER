...
def start_register(message: Message, state: StateContext):
    ...
    bot.send_message(message.chat.id, "📧 Введите ваш email, который вы использовали при регистрации на сайте:")
...
def get_password(message: Message, state: StateContext):
    ...
    if register_user_by_email_password(user_id, email, password):
        bot.send_message(message.chat.id, "✅ Вы успешно привязали Telegram аккаунт! Теперь вы можете получать уведомления и управлять задачами.")
    else:
        bot.send_message(message.chat.id, "❌ Неверный email или пароль. Попробуйте ещё раз /register")
    state.delete()